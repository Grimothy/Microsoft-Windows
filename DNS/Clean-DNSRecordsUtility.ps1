...
function Remove-STATICDNSRecord{
    ...
    Write-Host -ForegroundColor Green "Looking for DNS Records older than"$(Get-Date).AddDays(-$days)
    Start-Sleep -Seconds 5
    $recordsfound = Get-DnsServerResourceRecord -ZoneName $ZoneName -RRType A | Out-GridView -PassThru
    $TotalItems=$recordsfound.count
    $CurrentItem = 0
    #$ErrorActionPreference = 'SilentlyContinue'
    If ($WithCaution -like "Yes") {
...
function DNSRemovalMenu
{
   param(
    ...
    if ($item -eq 3)
    {
        ...
        Write-Host -ForegroundColor Green 'selected'
        Start-Sleep -Seconds 1
        Remove-STATICDNSRecord -WithCaution Yes
    }
    if ($item -eq 4)
    {
        Write-Host -ForegroundColor Green 'Remove STATIC DNS Records ' -NoNewline;
        ...
        Write-Host -ForegroundColor Green 'selected'
        Start-Sleep -Seconds 1
        Remove-STATICDNSRecord -WithCaution No
    }
    if ($item -eq 5)
    {
